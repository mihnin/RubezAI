...
- **Архитектурный план:** `docs/design/iteration-8-chat.md` (v3) — принят архитектором **9.6/10 ГОТОВ К РЕАЛИЗАЦИИ** после 3 ревью (6 → 8.5 → 9.6).
- **Фазы (TDD):** Ф1 sanitizer-клиент → Ф2 storage (chat/audit/users + миграция dev-users) → Ф3 оркестратор → Ф4 HTTP/SSE + контракт.
- **Тесты:** интеграционный тест полного потока (два аудит-события); контрактные тесты sanitizer и `chat.schema.json`.
