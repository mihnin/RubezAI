...
Итерации 0–7 приняты архитектором; Итерации 4–7 доведены до 10/10.
Открытый техдолг: единый `LLM_API_KEY` на все openai-провайдеры —
пункт после MVP (см. `docs/PLAN.md`, секция «Технический долг»).
Следующая — Итерация 8 (чат-оркестрация `/api/chat`: sanitize → policy →
route → SSE-стрим → проверка ответа → audit).
