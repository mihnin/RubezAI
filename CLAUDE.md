...
**MVP завершён.** Итерации 0–16 реализованы. Backend (Go API +
Python sanitizer + Python worker) на **10/10** (Итерации 4–11 после
3–4 ревью каждая). Frontend (Vite + React 19 + RR v7 + TanStack Query
+ Zod + Tailwind v4) с 6 экранами по `docs/design/ui/*.md`: Login, Chat
(SSE), Documents, Policies, Models, Audit Log, Incidents. Все 15
критериев MVP закрыты; `docker compose up` поднимает весь стек.
README содержит минимальный e2e-сценарий проверки.
