...
## Observability — Prometheus /metrics (W4)

Все три сервиса экспортируют метрики на endpoint `GET /metrics` в
text-exposition формате Prometheus:

- **`rubezh-api`** (`internal/metrics/metrics.go`) — изолированный
  `CollectorRegistry`, 6 кастомных серий:
  `rubezh_api_chat_requests_total{decision,provider,outcome}`,
  `rubezh_api_chat_request_duration_seconds{provider}` (histogram),
  `rubezh_api_audit_events_total{event_type}`,
  `rubezh_api_sanitize_failures_total{stage,reason}`,
  `rubezh_api_throttle_events_total{kind,outcome}`,
  `rubezh_api_llm_router_providers` (gauge, обновляется при hot-reload).
  Плюс стандартные `go_*` и `process_*`.
- **`rubezh-sanitizer`** (`app/api/metrics.py`):
  `rubezh_sanitizer_requests_total{context,outcome}`,
  `rubezh_sanitizer_duration_seconds{context}`,
  `rubezh_sanitizer_detector_matches_total{detector,category}`.
- **`rubezh-worker`** (`app/main.py`):
  `rubezh_worker_documents_processed_total{outcome}`,
  `rubezh_worker_processing_duration_seconds`,
  `rubezh_worker_queue_loop_errors_total{stage}`,
  `rubezh_worker_db_pool_ready` (gauge).

Инструментация в `rubezh-api` подключается через узкий интерфейс
`chat.MetricsRecorder` (`internal/chat/metrics.go`) — это разрывает цикл
зависимостей `chat ↔ metrics`. `Orchestrator.WithMetrics(rec)` — DI
из `main.go`; по умолчанию `noopMetrics{}` (тесты не настраивают).

**Cardinality правило:** в labels — только закрытые enum'ы (`decision`
= `policy.Decision`, `event_type` ~10 фиксированных значений, `provider`
= имя из таблицы `model_providers`). **Никаких** `user_id`, `session_id`,
`request_id` в метриках — они разорвут индекс Prometheus.

**Защита /metrics (W4 MJ-1):** env `METRICS_SCRAPE_BEARER`. Пусто →
endpoint открыт (on-prem за reverse proxy/периметром). Непусто → требует
`Authorization: Bearer <value>`, constant-time compare. Уровень
sanitizer/worker не имеет встроенной защиты — закрывайте на уровне
proxy/network policy для multi-tenant deploy'ев.

**Классификация sanitize-ошибок (W4 MJ-2):** `classifySanitizeError`
(в `chat/orchestrator.go`) использует `errors.Is(context.DeadlineExceeded)`
+ `errors.As(*net.OpError)` + проверку `Timeout()`-interface'а, а не
`strings.Contains` (раньше — терял локализованные/обёрнутые ошибки).
String-fallback оставлен для legacy транспортов.

## Health probes (W2.4)

- `rubezh-api`: `GET /health` — единый liveness (используется docker
  HEALTHCHECK и self-mode `rubezh-api healthcheck`).
- `rubezh-worker`: `GET /live` (без БД, k8s liveness),
  `GET /ready` (БД + `SELECT 1` с 2s timeout, k8s readiness, 503 при
  недоступности pool'а); `GET /health` — backward-compat alias `/live`.
- `rubezh-sanitizer`: `GET /health`.

Lifespan worker'а **tolerable**: при недоступной БД `pool=None`,
queue-loop не стартует, сервис не падает — `/ready` возвращает 503
пока БД не появится.

## Throttle (in-memory) и scale-ready путь

`internal/chat/throttle.go::throttleReporter` — in-memory token-bucket
per user, используется в двух местах:

- `ragPolicyRevisedReporter` (10/час) — для `policy_revised_after_rag`
  (Итерация 11 §Р4);
- `previewTokenMissReporter` (5/мин) — для дедупа `preview_token_miss`
  (W3.2, MJ-3 ревью W2).

**KNOWN LIMITATION:** state переживает рестарт сервиса — нет, не
переживает; in-memory. Для on-prem MVP с одним инстансом приемлемо.

При горизонтальном масштабировании (HA-deploy ≥2 инстансов) — переход
на Postgres advisory locks / таблицу `throttle_buckets` по плану в
`docs/design/scalable-throttle.md` (W4.3 — design-doc, реализация при
наступлении триггеров; в design-doc есть SQL миграция, алгоритм
атомарного UPSERT и интерфейс `ThrottleStore`).
...
**Production-ready RC: tag `v0.9.0-rc`** (commit `bb9890f`, push в
origin/main). Итерации 0–16 + E/F/H/H.3/G.1/G.2/RAG + SSH-CLI bridge
(000017–000019) + Live status SSE + **post-MVP волны W1–W4** реализованы;
все слои подтверждены живым e2e через `docker compose up` (без mock'ов
на критическом пути). Архитектор: 9.5/10, готовность к production v1.0
≈ 95% (перед v1.0 — закрыть `/metrics` через `METRICS_SCRAPE_BEARER`
или nginx-rule; см. observability-секцию выше).

**Volнs W1–W4 (post-MVP polish после внешнего аудита 8 finding'ов):**

- **W1 — Security P1:** RBAC + sanitize + sha256-audit для `system_prompt`
  и `review.system_prompts` (admin/dev-only, 403 иначе); allow_raw +
  preview_token восстанавливает sanitized document body через
  `pmap.Restore` вместо плейсхолдера.
- **W2 — UX/Stability P2:** SSE truncation guard в `sse.ts`; review-loop
  видит файлы (text-preview через pmap.Remask, защита от PII); worker
  `/live`/`/ready` с 2s timeout; testdb cleanup (482→136 providers);
  `preview_token_miss` audit + graceful fallback на default review
  prompt при ошибке sanitize.
- **W3 — Contract sync + docs:** `sanitize.schema.json` расширен 4
  контекстами (`chat|document|system_prompt|review_system_prompt`),
  `preview_token_miss` throttle 5/мин, синхронизация документации
  (README/API.md/ARCHITECTURE.md/PLAN.md/CLAUDE.md).
- **W4 — Observability + production polish:** Prometheus `/metrics` на
  всех трёх сервисах (см. секцию Observability выше), Bearer-защита
  `/metrics`, `classifySanitizeError` через `errors.Is/As`, тесты на
  attachment regex (2 файла подряд, base64 newlines), design-doc
  `scalable-throttle.md` для HA.

Параллельный файл инструкций для Codex/других агентов — `AGENTS.md`
(зеркало этого CLAUDE.md, держим синхронным при правках).
