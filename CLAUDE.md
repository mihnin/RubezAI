...
**Backend MVP — полностью на 10/10.** Итерации 0–11 приняты архитектором;
Итерации 4–11 доведены до **10/10** (3–4 ревью каждая). Этап A (UX/UI) — **10/10**.
Критерии MVP: 3, 4, 5, 6, 7, 8, 9, 10, 11 закрыты (9 из 15).
**Frontend** — Итерация 12 (каркас) реализована: Vite + React 19 +
RR v7 + TanStack Query + Zod + Tailwind v4, dark-first по
`docs/design/ui-system.md`; Docker-сборка проверена. Остаются
**Итерации 13–15** (6 экранов по `docs/design/ui/*.md`) и
**Итерация 16** (e2e интеграция). Эти итерации закроют
критерии 1, 2, 12, 14, 15.
