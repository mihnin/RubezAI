...
F + H + H.3 + G.1 + G.2 + Итерация 11 (RAG) реализованы; backend и
frontend подтверждены реальным e2e-прогоном через `docker compose up`
(без mock'ов на критическом пути).
